...
write(21,'(13a)') 'P_val ',char(9),'T_val ',char(9),'x_val ',char(9),'y_val ',char(9),'y_calc',&
	&char(9), 'deviation ', char(9),' abs_dev '
do i=1,nex
	...
	&,char(9),(x(i)*xmax-ftest1)/(x(i)*xmax)
	rmsav=rmsav+(x(i)*xmax-ftest1*xmax)*(x(i)*xmax-ftest1*xmax)
	absdev=absdev+(x(i)*xmax-ftest1*xmax)
	if (x(i)/=0.0) then
		reldev=reldev+(x(i)-ftest1)/(x(i))
		reln=reln+1
	endif
enddo
...
write(21,'(a,f20.10)') 'average root mean square deviation ', sqrt(rmsav)
write(21,'(a,f20.10,a)') 'average relative deviation ', reldev *100.0 , '  %'
write(21,'(a,f20.10)') 'average absolut deviation', absdev
print *, 'half ok'
do i=1,k+1
	do ii=1,m+1
		do iii=1,r+1
